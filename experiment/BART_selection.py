...
def run_BART(
    ...
    print(40 * "=", "Evaluating BART on ", dataset, 40 * "=", sep="\n")
    ...
    if sym_data:
        ...
        true_feature_names = feature_names[range(p0)]
        if og_data:
    ...
    if signal_to_noise > 0:
        ...
        labels += np.random.normal(
            ...
            # scale=np.exp(
            #     (np.log(np.var(labels)) - np.log(signal_to_noise)) / 2
            # ),
            size=len(labels),
        )
        var_eps = np.var(labels) - var_f
    else:
        var_eps = 1e-18
    ...
    try:
        ...
        if est_name == "BART_VIP_Rank":
            vip = list(result[0])
            vip_rank = list(result[1])
            vc = list(result[2])
            vc_rank = list(result[3])
        elif est_name == "DART_VC-measure":
            vip = list(result[0])
            vip_rank = list(result[1])
            vc = list(result[2])
            vc_rank = list(result[3])
            s = list(result[4])
        elif est_name == "ABC_Bayesian_forests":
            s = list(result[0])
        elif est_name == "BART_MI":
            idx_vip = list(result[0])
            idx_mi = list(result[1])
            mi = list(result[2])
        else:
